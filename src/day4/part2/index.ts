...
for (const winNumber of winningNumbers) {
  ...
  // if a single board is left, calculate the result
  if (boards.length === 1 && checkBoardWin(boards[0])) {
    console.log("Result: ", sumBoardNonWinningNumber(boards[0]) * winNumber);
    break;
  }
  // then remove boards which have won
  boards = boards.filter((b) => !checkBoardWin(b));
}
