...
{
  const int destinationCrateCount = strlen(piles[to].crates);
  const int sourceCrateCount = strlen(piles[from].crates);

  // copy the crates from the source to the destination
  memcpy(piles[to].crates + destinationCrateCount, piles[from].crates + sourceCrateCount - count, count);

  // remove the crates from the source
  memset(piles[from].crates + sourceCrateCount - count, 0, count);
}
